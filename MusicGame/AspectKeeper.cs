using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[ExecuteAlways]
public class AspectKeeper : MonoBehaviour
{

    [SerializeField]private Camera targetCamera; //対象とするカメラ
    [SerializeField] private Vector2 aspectVec; //目的解像度

    void Update()
    {

        var screenAspect = Screen.width / (float)Screen.height; //いまの画面のアスペクト比
        var targetAspect = aspectVec.x / aspectVec.y; //目的のアスペクト比

        var magRate = targetAspect / screenAspect; //目的アスペクト比にするための倍率
        var viewportRect = new Rect(0, 0, 1, 1); //Viewport初期値でRectを作成 幅1　高さ1

        if (magRate < 1)//今が横長すぎるから縦長にして調節したい-横軸は一定にする
        {
            viewportRect.width = magRate; //使用する横幅を変更　
            viewportRect.x = 0.5f - viewportRect.width * 0.5f;//中央寄せ
        }//高さを1として調整

        else
        {
            viewportRect.height = 1 / magRate; //使用する縦幅を変更
            viewportRect.y = 0.5f - viewportRect.height * 0.5f;//中央余生
        }//高さを1として調整

        targetCamera.rect = viewportRect; //カメラのViewportに適用

    }

}